...
public interface RelativeRepository extends JpaRepository<Relative, Long> {
    ...
    @Query("SELECT r FROM Relative r JOIN Child c WHERE r.id = ?1 AND c.id = ?2 AND c.group.teacher.email = ?3")
    Optional<Relative> findRelativeByIdAndChildIdAndTeacherEmail(long relativeId, long kidsId, String email);

    @Query("SELECT r FROM Relative r JOIN Child c WHERE c.id = ?1 AND c.group.teacher.email = ?2")
    List<Relative> findRelativesByChildIdAndTeacherEmail(Long childId, String teacherEmail);

    @Query("SELECT r FROM Relative r WHERE r.name = ?1 AND r.address = ?2 AND r.phone = ?3")
    Optional<Relative> findEqualRelative(String name, String address, String phone);

    @Query("SELECT r FROM Relative r WHERE r.name = ?1 AND r.address = ?2 AND r.phone = ?3 AND r.id <> ?4")
